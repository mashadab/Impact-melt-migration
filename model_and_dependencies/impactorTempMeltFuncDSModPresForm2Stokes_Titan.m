function impactorTempMeltFuncDSModPresForm2Stokes_Titan(fn,eta_0,E_a,kc)
    %{
    ...
    Variables:
        ...
        outputs from the impact simulation)i%Testing purposes only
        ...
        kc: is the permeability coefficient with units in m^2
    ...
    XX = linspace(0,Xmax*1e-3,50);  %in km
    YY = linspace(0,Ymax*1e-3,150);  %in km
    [Xf,Yf] = meshgrid(XX,YY);  %in km

    T   = (griddata(T_Shigeru(:,1),T_Shigeru(:,2),T_Shigeru(:,3),Xf,Yf));
    phi = (griddata(phi_Shigeru(:,1),phi_Shigeru(:,2),phi_Shigeru(:,3),Xf,Yf)/100);
    phi(phi<0) = 0;
    %phi(phi>0) = 1;
   
    
    %surf(T_Shigeru(:,1),T_Shigeru(:,2),T_Shigeru(:,3))
...
%             if i<1500
            ...
            tt.TileSpacing = 'none';
            set(gca,'Layer','top')
            saveas(hh,sprintf('../figures/newres_test__fig%d_imp_kc%d.png',i,kc)); 
            saveas(hh,sprintf('../figures/pdfnewres_test__fig%d_imp_kc%d.pdf',i,kc)); 
